---
title: Aprendizado
navigation: Aprendizado
slug: /learning
description: Aprendizado contínuo, certificações, treinamentos e desenvolvimento profissional.
language: pt-br
status: approved
lastUpdated: 2026-08-05
heroEyebrow: Aprendizado
heroSubtitle: Evolução contínua em arquitetura, nuvem e ecossistema Adobe.
heroDescription: Estudos, certificações e experimentos práticos que mantêm as decisões técnicas atuais e aplicáveis.
quickLinks:
  - title: Produção Técnica
    description: Artigos em que aprendizados viram orientação prática para engenharia.
    href: /writing
    ctaLabel: Ler insights
  - title: Carreira
    description: Linha do tempo mostrando como o estudo contínuo influenciou a trajetória.
    href: /career
    ctaLabel: Ver trajetória
  - title: Contato
    description: Trocar experiências sobre certificações, estudo e comunidades técnicas.
    href: /contact
    ctaLabel: Conectar
---

Aqui estão os focos atuais de estudo e como esse aprendizado é aplicado em decisões técnicas e entregas concretas.

## Áreas de Estudo Atuais

Atualmente concentro meus estudos principalmente em:

- Adobe Experience Cloud
- Adobe Experience Manager
- Adobe Experience Platform
- Edge Delivery Services
- Inteligência Artificial
- IA Generativa
- Arquitetura de Software
- Arquitetura em Nuvem
- DevOps
- Engenharia de Software Moderna

---

## Certificações

### Blip Academy

### Certificado de Conclusão – Plataforma Blip

**Emitido:** Fevereiro de 2025

Certificação de conclusão da plataforma Blip.

---

### Desenvolvimento no Blip | Technical Essentials para Parceiros

**Emitido:** Março de 2025

Treinamento técnico voltado ao desenvolvimento de soluções utilizando a plataforma Blip.

---

### Análise de Dados | Technical Essentials para Parceiros

**Emitido:** Março de 2025

Treinamento focado em análise de dados e recursos da plataforma.

---

### Blip – Experiência do Usuário (UX)

**Emitido:** Março de 2025

Treinamento sobre conceitos de UX e boas práticas para construção de experiências conversacionais.

---

### Adobe

#### Adobe Real-Time Customer Data Platform (RT-CDP) Bootcamp

Treinamento focado em Adobe Experience Platform e Real-Time Customer Data Platform.

---

### Outros Treinamentos

Ao longo da carreira participei de diversos cursos e programas de capacitação, entre eles:

- Academia do Java
- Gestão de Projetos com Scrum
- Imersão LGPD
- Técnicas de Computação Forense (4Linux)
- Arduino Básico
- Desenvolvimento de Software
- Arquitetura de Sistemas
- Computação em Nuvem

---

### Filosofia de Aprendizado

Acredito que aprender deve fazer parte da rotina de qualquer profissional de tecnologia.

Cada nova tecnologia representa uma oportunidade de melhorar processos, simplificar soluções e ampliar a capacidade de resolver problemas complexos.

Para mim, estudar não é uma atividade separada do trabalho.

É parte do processo de engenharia.

---

### Compartilhando Conhecimento

Aprender também significa compartilhar.

Além dos estudos, procuro transformar conhecimento em artigos técnicos, documentação de arquitetura, provas de conceito e discussões com equipes de desenvolvimento.

Ensinar é uma das formas mais eficientes de consolidar o próprio aprendizado.

---

### Explorando Atualmente

Atualmente tenho dedicado parte dos meus estudos aos seguintes temas:

- Inteligência Artificial aplicada à Engenharia de Software
- Desenvolvimento Assistido por IA
- Gestão do Conhecimento em Engenharia
- Adobe Experience Cloud
- Plataformas Digitais Modernas
- Arquiteturas Cloud Native
- Padrões de Arquitetura de Software

---

## Continue Explorando

Quer ver como esses conhecimentos são aplicados em projetos reais?

[Estudos de Caso](/pt-br/case-studies/)

Ou conhecer os artigos técnicos onde compartilho pesquisas, aprendizados e experiências práticas?

[Produção Técnica](/pt-br/writing/)

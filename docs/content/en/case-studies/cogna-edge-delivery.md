---
title: Cogna – Edge Delivery Platform
slug: /case-studies/cogna-edge-delivery
description: Building a scalable landing page platform with Adobe Edge Delivery Services for one of Brazil's largest education groups.
language: en
status: approved
lastUpdated: 2026-08-05
---

Accelerating Marketing Campaign Delivery with Adobe Edge Delivery Services


## Overview

Cogna, one of Brazil's largest education groups, frequently launches marketing campaigns for enrollment, re-enrollment and seasonal promotions across multiple educational brands.

Traditional website development workflows could not keep pace with the speed required by the marketing teams.

The objective of the project was to create a reusable landing page platform using **Adobe Edge Delivery Services (EDS)**, enabling rapid content creation while maintaining governance, design consistency and performance.


## Client

**Cogna Educação**

Industry: Education


## Company

**Netbiis**


## My Role

- Software Architect
- Tech Lead
- Systems Analyst

I was responsible for solution architecture, technical leadership, platform design and engineering coordination throughout the project.


## Business Challenge

Marketing teams needed to publish new landing pages within very short deadlines.

The platform had to provide:

- Fast campaign delivery.
- Visual consistency.
- Brand governance.
- Easy content editing.
- High performance.
- Reusable components.
- Minimal technical dependency.

The solution also needed to support multiple educational brands while reducing development effort.


## Solution

A reusable landing page platform was built using **Adobe Edge Delivery Services**.

Instead of developing each landing page individually, a library of reusable blocks and predefined layouts was created.

Marketing teams could assemble new pages using approved components while developers maintained full control over the platform architecture.

This approach significantly reduced development time and improved consistency across campaigns.


## Technologies

### Adobe

- Adobe Edge Delivery Services
- Universal Editor
- Adobe Experience Manager as a Cloud Service
- Document-Based Authoring

### Engineering

- HTML
- CSS
- JavaScript
- Git
- GitHub
- CI/CD


## Architecture Responsibilities

My responsibilities included:

- Solution Architecture
- Platform Design
- Technical Leadership
- Requirements Analysis
- Component Architecture
- Development Standards
- Performance Strategy
- Technical Coordination


## Key Challenges

One of the main challenges was balancing flexibility for marketing teams with technical governance.

The platform needed to allow rapid content creation while ensuring that every landing page followed design standards, performance requirements and accessibility guidelines.

Another challenge was designing reusable blocks that could support different campaigns without requiring custom development for every new landing page.


## Results

The project delivered:

- Reusable landing page platform.
- Standardized component library.
- Faster campaign launches.
- Reduced development effort.
- Improved design consistency.
- Better website performance.
- Simplified content authoring.

The platform established a scalable foundation for future marketing initiatives across multiple educational brands.


## Engineering Takeaways

Reusable platforms create far more value than isolated projects.

By investing in architecture and reusable components, organizations reduce delivery time, improve consistency and enable teams to scale their digital operations more efficiently.


## Technologies Related

- Adobe Edge Delivery Services
- Universal Editor
- Adobe Experience Manager
- Digital Experience Platforms
- Front-End Architecture
- Software Architecture


## Related Content

### Related Expertise

- Adobe Experience Cloud
- Digital Experience Platforms
- Software Architecture
- Front-End Engineering


### Related Articles

- [Adobe Edge Delivery Services: A Technical Perspective for Developers](/en/writing/#adobe-edge-delivery-services-a-technical-perspective-for-developers)
- [CORS in AEM as a Cloud Service for GraphQL (POST): What the Documentation Doesn't Tell You](/en/writing/#cors-in-aem-as-a-cloud-service-for-graphql-post-what-the-documentation-doesnt-tell-you)


## Next Case Study

[Irani Financial Reports Platform](/en/case-studies/irani/)

---
title: Portal O TEMPO
slug: /case-studies/o-tempo
description: Enterprise migration to Adobe Experience Manager as a Cloud Service for one of Brazil's largest digital news platforms.
language: en
status: approved
lastUpdated: 2026-08-05
---

Modernizing a Large-Scale Digital News Platform with Adobe Experience Manager


## Overview

O TEMPO is one of Brazil's leading digital newspapers, publishing thousands of articles every day across multiple editorial sections.

The project aimed to modernize the entire digital publishing platform by migrating to **Adobe Experience Manager as a Cloud Service**, providing journalists and editors with a modern authoring experience while improving platform scalability, maintainability and long-term evolution.


## Client

**Grupo O TEMPO**

Industry: Media


## Company

**Netbiis**


## My Role

- Software Architect
- Tech Lead
- Systems Analyst

I was responsible for the technical architecture, solution design, engineering leadership and coordination of the implementation throughout the project lifecycle.


## Business Challenge

The project involved much more than replacing a CMS.

The platform needed to:

- Migrate nearly two million existing articles.
- Preserve historical content and metadata.
- Support a high-volume publishing operation.
- Improve the editorial workflow.
- Adapt Adobe Experience Manager to the newsroom's daily processes.
- Integrate with existing publishing services.
- Maintain high availability and performance.


## Solution

A new enterprise platform was designed using Adobe Experience Manager as a Cloud Service as the foundation for the digital publishing ecosystem.

The solution included:

- Modern authoring experience.
- Cloud-native architecture.
- Content migration strategy.
- Content modeling.
- Integration services.
- Performance optimization.
- CI/CD pipelines.
- Monitoring and operational support.


## Technologies

### Adobe

- Adobe Experience Manager as a Cloud Service
- Adobe Edge Delivery Services
- Universal Editor
- AEM Assets
- Content Fragments
- Experience Fragments
- GraphQL
- Cloud Manager
- Adobe App Builder
- Adobe I/O

### Engineering

- Java
- HTL
- Sling Models
- OSGi
- Oak Indexes
- Lucene
- Maven
- Git
- CI/CD


## Architecture Responsibilities

My responsibilities included:

- Solution Architecture
- Technical Leadership
- Requirements Analysis
- System Design
- Integration Architecture
- Code Review
- Development Standards
- Performance Analysis
- Deployment Strategy
- Technical Support


## Key Challenges

One of the most significant challenges was migrating nearly two million articles while preserving their structure, metadata and relationships.

Another major challenge was adapting Adobe Experience Manager to the newsroom workflow, ensuring editors could work efficiently without compromising governance or content quality.

The project also required balancing performance, scalability and maintainability while supporting a continuously evolving publishing operation.


## Results

The project successfully delivered:

- Migration of nearly 2 million articles.
- Modern Adobe Experience Manager Cloud platform.
- Improved editorial workflow.
- Centralized content management.
- High-performance publishing platform.
- Scalable cloud architecture.
- Long-term maintainability.

The new platform provided a strong foundation for the continuous evolution of O TEMPO's digital ecosystem.


## Engineering Takeaways

Large enterprise migrations are not simply technology replacement projects.

Success depends on understanding business processes, content architecture, operational workflows and long-term platform evolution.

Technology is only one part of the solution.


## Technologies Related

- Adobe Experience Manager
- Adobe Experience Cloud
- Adobe Edge Delivery Services
- Cloud Architecture
- Software Architecture
- Technical Leadership


## Related Content

### Related Expertise

- Adobe Experience Cloud
- Software Architecture
- Digital Experience Platforms


### Related Articles

- [Understanding Full-Text Search in AEM](/en/writing/#understanding-full-text-search-in-aem-how-lucene-really-works)
- [Understanding AEM Index Naming and Versioning](/en/writing/#understanding-aem-index-naming-and-versioning)
- [includedPaths vs queryPaths](/en/writing/#includedpaths-vs-querypaths-understanding-index-scope-in-aem)
- [Adobe Edge Delivery Services: A Technical Perspective](/en/writing/#adobe-edge-delivery-services-a-technical-perspective-for-developers)


## Next Case Study

[Palmeiras Digital Assets](/en/case-studies/palmeiras/)
